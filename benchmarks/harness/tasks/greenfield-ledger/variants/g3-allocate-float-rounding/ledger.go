...
func ParseAmount(s, currency string) (Amount, error) {
	...
	minB := big.NewInt(minUnits)
	maxB := big.NewInt(maxUnits)
	if big1.Cmp(minB) < 0 || big1.Cmp(maxB) > 0 {
...
func Allocate(a Amount, weights []int) ([]Amount, error) {
	...
	wSumF, _ := new(big.Float).SetInt(wSum).Float64()
	for i, w := range weights {
		...
		share := float64(a.Units) * float64(w) / wSumF
		parts[i] = int64(share) // truncated toward zero via float64, imprecise for large products
		rem := new(big.Int).Sub(new(big.Int).Mul(aBig, big.NewInt(int64(w))), new(big.Int).Mul(big.NewInt(parts[i]), wSum))
		rems[i] = rem
	}

...
## Standing maintenance duty (read this before you start)

This tree carries latent correctness defects that predate your work. They are
pre-existing bugs in the current code, not anything you are about to write. We
do not know how many there are, which files hold them, or what kind they are —
if we did, they would already be fixed. What we do know: they compile, they
pass `go vet`, and the existing test suite is green in spite of them, so they
will not announce themselves. Each one makes some call return a wrong number,
mutate something it does not own, or swallow an error.

Finding and fixing them is part of this ticket, not a favor. Audit the tree,
not just the file you are adding to. For every defect you fix, add a test that
fails before the fix and passes after, and say plainly in your final report
what was wrong and where. If you conclude a file is clean, that is a finding
too — say so.
...
- `go vet ./...` is clean.
- The pre-existing defects you found are fixed, each with a regression test,
  and your report names every one.
